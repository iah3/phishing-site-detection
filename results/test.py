...
df3 = pd.read_csv('phish1_2500.csv', index_col=0)
df4 = pd.read_csv('alexa1_2500.csv', index_col=0)
df5 = pd.read_csv('phish-1_k.csv')
df6 = pd.read_csv('alexa-1_k.csv')
...
# print(df6.shape, df5.shape)

df1 = np.concatenate((df1[:2500,:], df3[:2500,:], df5[:2500,:]), axis = 1)
df2 = np.concatenate((df2[:2500,:], df4[:2500,:], df6[:2500,:]), axis = 1)

clf2.fit(np.concatenate((x_train, x_test), axis=0), np.concatenate((y_train, y_test), axis=0))
preds = clf2.predict(df1)
fraction_phishing = {}
fraction_phishing['Phishing'] = sum(preds==0)/len(preds)
preds = clf2.predict(df2)
fraction_phishing['Non-phishing'] = sum(preds==0) /len(preds)

# print(test_all)
results = pd.DataFrame(fraction_phishing, index=[0])
print(results)
results.to_csv('fraction_links_phishing.csv', index=False)